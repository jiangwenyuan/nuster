...
int _nst_cache_stats_head1(struct appctx *appctx, struct stream *s,
...
int _nst_cache_stats_data1(struct appctx *appctx, struct stream *s,
        struct stream_interface *si, struct channel *res) {
            ...
            if(!LIST_ISEMPTY(&p->nuster.rules)) {
                ...
                list_for_each_entry(rule, &p->nuster.rules, list) {
                    ...
                    if(rule->uuid == appctx->st2) {
                        ...
                        if(!htx_add_data_atonce(htx, ist2(trash.area,
                                        trash.data))) {
...
static void nst_cache_stats_handler1(struct appctx *appctx) {
    ...
    if(appctx->st0 == NST_CACHE_STATS_HEAD) {
        ...
        if(_nst_cache_stats_head1(appctx, s, si, res)) {
    ...
    if(appctx->st0 == NST_CACHE_STATS_DATA) {
        ...
        if(_nst_cache_stats_data1(appctx, s, si, res)) {
...
static void nst_cache_stats_handler2(struct appctx *appctx) {
    struct stream_interface *si = appctx->owner;
...
static void nst_cache_stats_handler(struct appctx *appctx) {
    ...
    if (IS_HTX_STRM(s)) {
        return nst_cache_stats_handler2(appctx);
    } else {
        return nst_cache_stats_handler1(appctx);
    }
}
