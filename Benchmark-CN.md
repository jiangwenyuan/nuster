性能测试: nuster vs nginx vs varnish
=================================================

主要用`wrk`来测试nuster, nginx和varnish的每秒请求数。

结果显示单进程模式下差不多是nginx的3倍，多进程下nginx的2倍，varnish的3倍。
...
两个Linux服务器, server129包含wrk和一个后端web服务器,
nuster/nginx/varnish都在server130.
...
在后端web服务器上设置 set `server_tokens off;` 使请求头的server一致。
...
* 1 core
  ...
  * nuster差不多是nginx3倍
* 12 cores
  * 请求大于256字节的文件时几乎占满宽带(see [Raw output](#raw-output))
  * 没占满带宽的时候nuster差不多是nginx的2倍，varnish的3倍
  * 占满: 都差不多

因为没有10Gbps网络用127.0.0.1再测试一遍
